...
import styles from './waterBall.less'
const d3  = require('d3');
...
export default class Main extends Component{
  ...
  liquidFillGaugeDefaultSettings(){
    ...
    return {
      ...
      textColor: props.textColor || "#A4DBf8", // The color of the value text when the wave does not overlap it.
  ...
  render(){
    ...
    const { title, textColor, waveTextColor } = config
    const { value } = data

    return(
      <div className={styles['app-circleBall']}>
        <div id={idDom}></div>
        <h3 className={styles.title} style={{color: `${value > 0.4 ? waveTextColor : textColor}`}}>{title}</h3>
      </div>
    );
  }
}
